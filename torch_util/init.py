...
def deep_init_params(v: Any, func: Callable):
    if isinstance(v, Tensor):
        func(v)
    elif hasattr(v, 'parameters'):
        for param in v.parameters():
            deep_init_params(v=param, func=func)
    else:
        raise TypeError 


def xavier_uniform_(v: Any, gain: float = 1.):
    deep_init_params(v=v, func=lambda x: xavier_uniform_(x, gain=gain))
    
    
def xavier_normal_(v: Any, gain: float = 1.):
    deep_init_params(v=v, func=lambda x: xavier_normal_(x, gain=gain))
